...
Below are some notes on how to build Whippet Core for Windows.
...
Note that for WSL v1 the Whippet Core source path MUST be somewhere in the default mount file system, for
example /usr/src/whippet, AND not under, for example, /mnt/d/whippet.
...
```
Ensure that this toolchain can build for `posix` (else you may have compilation errors for Whippet's dependencies):
```bash
...
way. This will install to `c:\workspace\whippet`, for example:
```bash
make install DESTDIR=/mnt/c/workspace/whippet
```
