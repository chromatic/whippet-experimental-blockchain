Whippet Fee Recommendation
----------------------------

_last updated for 1.14.6_

The Whippet chain has a relatively low block interval, 1 megabyte blockspace
and aims to provide a cheap means for people to transact. Therefore, the biggest
threat to the Whippet chain as a whole is spam and in 2014, a transaction fee
and dust disincentive were introduced, to combat on-chain spam.

Whippet Core implements a number of defaults into the software that reflect the
...
parametrized. The recommended defaults, as implemented in the Whippet Core
wallet, are:

- **0.01 WHT per kilobyte** transaction fee
- **0.01 WHT** dust limit (discard threshold)
- **0.001 WHT** replace-by-fee increments

The wallet rejects transactions that have outputs under the dust limit, and
discards change to fee if it falls under this limit.

Note: In the past, Whippet has enforced a rounding function in the fee
      ...
      transaction only has to pay `0.01 / 1000 * 192 = 0.00192` WHT fee.

## Miner default inclusion policies

The default values for miners to include a transaction in a block has been set
to exactly the recommended fee of **0.01 WHT/kB.** Dust limits are defined by
...
The default minimum transaction fee for relay is set at **0.001 WHT/kB**,
...
- The hard dust limit is set at **0.001 WHT** - outputs under this value are
  invalid and rejected.
- The soft dust limit is set at **0.01 WHT** - sending a transaction with outputs
  under this value, are required to add 0.01 WHT for each such output, or else
