...
The data directory is the default location where the Whippet Core files are stored.
...
Windows  | `%APPDATA%\Whippet\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for the `whippet.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `whippet.conf`       | User-defined configuration settings for `whippetd` or `whippet-qt`; can be specified by `-conf` option
`./`               | `whippetd.pid`        | Stores the process ID (PID) of `whippetd` or `whippet-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `whippetd` or `whippet-qt`
...
`whippet-qt` uses the [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Whippet Core:
