...
// Vendored so uap.js works identically in browsers and Node without
// branching on WebCrypto vs. Node's crypto module.
//
// Verified against the standard NIST test vectors in sha256.test.js.
...
function sha256(message) {
  ...
  const padLen = ((message.length + 8) >> 6) * 64 + 64;
...
/**
 * Synchronous HMAC-SHA256 (RFC 2104), built on the sha256() above.
 * Verified against the RFC 4231 test vectors in sha256.test.js.
 * Accepts any number of message parts, concatenated before hashing.
 */
function hmacSha256(key, ...messageParts) {
...
export { sha256, hash256, hmacSha256, concatBytes };
