...
#if defined(NDEBUG)
# error "Whippet cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Whippet Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetWhippetMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Whippet: Switch maturity at depth 145,000
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Whippet: BIP30 has been active since inception
    ...
    // Whippet: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetWhippetBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Whippet: Disallow AuxPow blocks before it is activated.
    ...
    if (consensusParams.nMinBlockSpacing > 0 &&
        block.GetBlockTime() < pindexPrev->GetBlockTime() + consensusParams.nMinBlockSpacing)
        return state.DoS(100, error("%s: block timestamp violates minimum spacing", __func__),
                         REJECT_INVALID, "bad-time-too-early");
    ...
    // Whippet: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Whippet: We probably want to disable this
    int nLockTimeFlags = 0;
