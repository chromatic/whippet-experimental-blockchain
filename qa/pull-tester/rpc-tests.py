...
if 'ENABLE_WALLET' not in vars():
    ENABLE_WALLET=0
if 'ENABLE_WHTCOIND' not in vars():
    ENABLE_WHTCOIND=0
...
#Set env vars
if "WHTCOIND" not in os.environ:
    os.environ["WHTCOIND"] = BUILDDIR + '/src/whippetd' + EXEEXT
...
if not (ENABLE_WALLET == 1 and ENABLE_UTILS == 1 and ENABLE_WHTCOIND == 1):
    print("No rpc tests to run. Wallet, utils, and whippetd must all be enabled")
    sys.exit(0)
...
class RPCTestHandler:
    ...
    def __init__(self, num_tests_parallel, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie whippetds, we can apply a
...
class RPCCoverage(object):
    ...
    commands per `whippet-cli help` (`rpc_interface.txt`).
